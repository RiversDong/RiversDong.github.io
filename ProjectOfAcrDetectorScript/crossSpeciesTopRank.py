...
def Top10Inacrs(acrs_id):
    ...
    if f:
        for i in f:
            acr=i.split()[0]
            probability=i.split()[1]
            if acr in acrs_id:
                info.append([acr, probability, "In",rank])
            else:
                info.append([acr,probability,"Out",rank])
            rank=rank+1
    else:
        for i in acrs_id:
            info.append([i,"NA","NA","NA"])
    return info
...
for i in assembly178:
    ...
    if cds in allCdsFiles:
        ...
        os.system(cmd)
        info=Top10Inacrs(acrs_id)
        for ii in info:
            print(i+"\t"+"\t".join(map(str,ii)))
